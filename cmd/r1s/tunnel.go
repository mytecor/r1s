...
// relayPort is moved to tunnel_relay.go: it is the low-level bidirectional
// byte pump between a local TCP connection and one authenticated tunnel stream
// (F19 client edge), independent of CLI parsing. It stays a localCLI method
// only to reach the shared tunnel session client.// tunnel is rejected in direct mode: a direct run would let the execution's
