...
func TestDescriptorRoundTrip(t *testing.T) {
	...
	descriptor, err := newDescriptor(clusterID, map[string]uint32{"gpu": 2, "default": 1}, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDescriptorCarriesNodeSummary(t *testing.T) {
	...
	descriptor, err := newDescriptor(clusterID, map[string]uint32{"default": 1}, node)
	if err != nil {
		t.Fatal(err)
	}
...
func TestDescriptorFitsAnnounceBudget(t *testing.T) {
	...
	descriptor, err := newDescriptor(clusterID, map[string]uint32{"default": 1, "gpu": 2}, realistic)
	if err != nil {
		t.Fatal(err)
	}
	data, err := descriptor.marshal()
	...
	descriptor, err = newDescriptor(clusterID, map[string]uint32{"a": 1, "b": 1, "c": 1, "d": 1}, wide)
	if err != nil {
		t.Fatal(err)
	}
