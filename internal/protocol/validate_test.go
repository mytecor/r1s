...
func TestValidateEveryPayload(t *testing.T) {
	...
	tests := []*r1sv1.Envelope{
		...
		envelope(now, &r1sv1.Envelope_ExecutionTunnelOpen{ExecutionTunnelOpen: &r1sv1.ExecutionTunnelOpen{
			ExecutionId: "execution", YggPeerPubkey: []byte("edge-node-public-key"),
			Targets: []*r1sv1.TunnelTarget{{Port: 22}},
		}}),
		envelope(now, &r1sv1.Envelope_ExecutionTunnelOpenAck{ExecutionTunnelOpenAck: &r1sv1.ExecutionTunnelOpenAck{
			ExecutionId: "execution", AllocatorEndpoint: []byte("transport-neutral-address"),
			AllocatorEndpointPubkey: []byte("transport-neutral-key"),
		}}),
	}
...
func TestValidateExecutionTunnelOpen(t *testing.T) {
	now := time.Unix(1_700_000_000, 0).UTC()
	valid := envelope(now, &r1sv1.Envelope_ExecutionTunnelOpen{ExecutionTunnelOpen: &r1sv1.ExecutionTunnelOpen{
		ExecutionId: "execution", YggPeerPubkey: []byte("edge-node-public-key"),
		Targets: []*r1sv1.TunnelTarget{{Port: 22}},
	}})
	tests := map[string]func(*r1sv1.Envelope){
		"missing execution ID": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpen().ExecutionId = ""
		},
		"missing peer public key": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpen().YggPeerPubkey = nil
		},
		"peer public key too large": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpen().YggPeerPubkey = bytes.Repeat([]byte{1}, tunnel.MaxPeerKeySize+1)
		},
		"missing targets": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpen().Targets = nil
		},
		"zero target port": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpen().Targets = []*r1sv1.TunnelTarget{{Port: 0}}
		},
		"target port too large": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpen().Targets = []*r1sv1.TunnelTarget{{Port: 65536}}
		},
	}
	if err := protocol.ValidateEnvelope(valid); err != nil {
		t.Fatalf("valid tunnel open rejected: %v", err)
	}
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			candidate := proto.Clone(valid).(*r1sv1.Envelope)
...
func TestValidateExecutionTunnelOpenAck(t *testing.T) {
	now := time.Unix(1_700_000_000, 0).UTC()
	valid := envelope(now, &r1sv1.Envelope_ExecutionTunnelOpenAck{ExecutionTunnelOpenAck: &r1sv1.ExecutionTunnelOpenAck{
		ExecutionId: "execution", AllocatorEndpoint: []byte("transport-neutral-address"),
		AllocatorEndpointPubkey: []byte("transport-neutral-key"),
	}})
	tests := map[string]func(*r1sv1.Envelope){
		"missing execution ID": func(envelope *r1sv1.Envelope) {
			envelope.GetExecutionTunnelOpenAck().ExecutionId = ""
		},
	}
	if err := protocol.ValidateEnvelope(valid); err != nil {
		t.Fatalf("valid tunnel open ack rejected: %v", err)
	}
	for name, mutate := range tests {
		t.Run(name, func(t *testing.T) {
			candidate := proto.Clone(valid).(*r1sv1.Envelope)
