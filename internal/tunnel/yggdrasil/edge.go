...
const (
	...
	// pendingSessionTimeout bounds how long a pending inbound session waits
	// for Accept before it is dropped.
	pendingSessionTimeout = 30 * time.Second
	// maxPendingSessions bounds the number of half-open inbound sessions, so
	// a flood of garbage packets from foreign keys cannot grow memory without
	// bound.
...
// Close stops the edge: sessions end classified as mesh-unreachable and the
